...
: 객체지향의 가장 기본적인 개념으로 관련된 속성과 동작을 하나의 범주로 묶어 실세계의 사물을 흉내 낸다. 

> 스크립트 언어는 원래 가볍게 쓰는 것이 주목적이어서 어렵고 난해한 객체지향을 지원하는 경우가 드물다. 파이썬은 스크립트 언어임에도 불구하고 C++과 모듈라3의 문법을 계승하여 간단한 클래스 정의뿐만 아니라 연산자 오버로딩, 다중상속까지 지원한다. 하지만 객체지향의 캡슐화, 추상화, 상속, 다형성 개념은 익숙해지는 데 상당한 시간과 실습이 필요하다. 

사물을 분석하여 필요한 속성과 동작을 추출하는 것을 **모델링**이라고 하며 모델링된 결과를 클래스로 포장하는 것을 **캡슐화**라고 한다.
